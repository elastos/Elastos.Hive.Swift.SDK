...
class HiveOneDriveFileTests: XCTestCase {
    ...
    override func setUp() {
        ...
        hiveDrive?.login(withResult: { (re, er) in
            self.lock?.fulfill()
...
//       try? self.hiveFile?.renameFileTo(newPath: "test2/", withResult: <#T##HiveResultHandler##HiveResultHandler##(Bool?, HiveError?) -> Void#>)
    }
